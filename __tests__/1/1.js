...
const exercise11 = (email) => {
  const initIndex = email.indexOf('@') + 1;
  return email.substring(initIndex, email.length - 4);
};
...
const exercise12 = (person) => {
  return person.subordinates.map(sub => {
    const initIndex = sub.email.indexOf('@') + 1;
    return sub.email.substring(initIndex, sub.email.length - 4);
  });
};
...
const exercise21 = (person, [minAge, maxAge]) => {
  return person.subordinates.reduce((prevVal, currVal) => prevVal + !!(currVal.age >= minAge && currVal.age <= maxAge), 0);
};
...
const exercise22 = (person) => {
  return person.subordinates.map(sub => {
    if(sub.subordinates.length) return sub.name;
  }).filter(Boolean);
};
...
const exercise31 = (person) => {
  return person.subordinates.reduce((prevVal, curr) => prevVal + curr.age, 0) / person.subordinates.length;
};
...
const exercise32 = (person) => {
  const numberOfFemales = person.subordinates.reduce((prevVal, curr) => prevVal + !!(curr.gender === "female"), 0);
  const numberOfMales = person.subordinates.reduce((prevVal, curr) => prevVal + !!(curr.gender === "male"), 0);
...
const exercise32a = (person) => {
  return person.subordinates.reduce((prevVal, curr) => prevVal + !!(curr.gender === "female") - !!(curr.gender === "male"), 0);
};
...
const reduce = (array, func, initalValue) => {
  let ret = initalValue;
  for(let a of array) ret = func(ret, a);
  return ret;
};
...
const exercise51 = (color) => {
  const personMain = CruzHarrell;
  let ret = personMain.eyeColor === color ? 1 : 0;

  function loopSubs(array) {
    let ret1 = 0;
    ret1 += array.reduce((prev, curr) => prev + !!(curr.eyeColor === color), ret1);
    for(let item of array) {
      if(item.subordinates.length) ret1 += loopSubs(item.subordinates);
    }
    return ret1;
  }

  ret += loopSubs(personMain.subordinates);
...
const exercise52 = (maxDistance) => {
  ...
  function loop(person) {
    let ans1 = person.subordinates.reduce((prev, curr) => prev + !!(distance(person.location, curr.location) <= maxDistance), 0);

    for(let item of person.subordinates) {
      if(item.subordinates.length) ans1 += loop(item);
    }
    return ans1;
  }
...
const exercise53 = () => {
  ...
  function loop(person) {
    let ans1 = person.subordinates.map(sub => {
      if(
        person.email.substring(person.email.indexOf('@') + 1, person.email.length - 4) === 
        sub.email.substring(sub.email.indexOf('@') + 1, sub.email.length - 4)
      ) 
        return sub.name.split(' ')[0];
      return undefined;
    }).filter(Boolean);

    for(let item of person.subordinates) {
      if(item.subordinates.length) ans1.push(...loop(item));
    }
    return ans1;
  }
